...
3) Import Todo.jsx into App.jsx
4) Use the Todo() component inside the <ul> element in the App() function.
5) Create a prop in Todo() in App.jsx which is called 'name'. Pass a random string into this prop.
6) Change the 'label' content of the 'task item input' in Todo() function as {props.name} to monitor the task item.
7) Make a DATA constant in index.js that shows the default task items in the main screen. That constant must be an object which contains 3 properties: id, name and completed. Completed will be a boolean.
8) Pass the DATA constant as a prop into the App() function in index.js. The prop name will be "tasks". This array is now available to the App component as props.tasks
...
10) Use the taskList constant instead of the children of the <ul> element, which list the task items.
...
12) Since keys should be unique, you can re-use the id of each task object as its key. Add the ids as key property to taskList constant.
...
3) Create a useState constant above the handleSubmit() function with [name, setName]. Initial state will be an empty string.
...
2) Make new useState constant with [tasks, setTasks]. The initial value ist 'props.tasks'. Now you can store the tasks in useState. That will be the main state tracker of the task list.
3) Now you can change the taskList constant that maps over the tasks. Here from now on you can map 'tasks' instead of 'props.tasks' because now prop.tasks = tasks main useState.

Adding a Task

1) Turning back to the addTask() function, you need to put the name into an object that has the same structure as your existing tasks. Inside of the addTask() function, you will make a 'newTask' object to add to the array. Because 'tasks(DATA)' is an array of objects and 'name' is a string.
2) Then in addTask() you will make a new array, with the existing tasks. And you will add the newTask into this array;  setTasks([newTask, ...tasks])
...
1) Make a headingText constant above the return statement of the App() function. This constant has to calculate the sum of taskList.
2) Now pass the headingText constant into the 'remaining tasks' part of the code.
3) When you have a single task you have to use the singular 'task' in the remaining tasks statement. To achieve that make the tasksNoun constant above headingText. This constant should include a simple ternary operator to use 'task' word if you have just one task.
4) Add the tasksNoun constant into the remaining tasks statement as a template literal.
...
2) That function has to change the state of the App() so that all tasks list has to re-render without the completed task. Assign a new constant into the function which is called updatedTasks.
...
4) Add toggleTaskCompleted() function as a property of <Todo /> which is rendered in taskList constant.
5) Now go to the 'input' element in the Todo.jsx component and add an onChange event listener. That event listener must trigger the toggleTaskCompleted(props.id) function with props.id. Remember that this function is also a prop of the Todo component.

Deleting a Task

1) You have to use a similar method with completing a task. Make the deleteTask(id) with an 'id' argument function under the toggleTaskCompleted() function.
2) Declare a constant which is called 'remainingTasks'. In this constant .filter() the tasks without the deleted task item.
3) Now pass the deleteTask() function as a prop in <Todo /> which is rendered in taskList constant.
...
2) Pass editTask as a prop in <Todo /> which is rendered in taskList constant.
3) To provide a UI to edit a task, import useState into Todo.jsx. Then create a useState hook with (false) initial, and [isEditing, setEditing] variables. 
...
5) Each template will be a constant under the useState hook and above the return statement.
...
8) Now in return statement of Todo() function you have to use a ternary operator, which decides whether the App uses the viewTemplate or editingTemplate. The isEditing variable with useState hook will help track the state.
...
10) Similarly, we can add an 'onClick' event listener to the 'cancel button' of editingTemplate, which changes the state of isEditing as 'false' with a callback function.
...
3) Using the Object.keys() method, collect the keys(properties) of the FILTER_MAP object into an array. And assign it to the FILTER_NAMES constant.
4) Underneath the taskList constant in App() make a new constant called filterList. That constant will map over the FILTER_NAMES as the 'key' and 'name' properties of <FilterButton /> component. So add the FilterButton() here, and instead of it add the filterList constant.
5) Now you have to update the <FilterButton /> in filterList by adding two new properties which will be isPressed and setFilter. The isPressed will check the conformity of 'name' and 'filter' with strict equality operators.
...
7) taskList constant maps the tasks in the App. To filter the tasks shown you have to add a filter before mapping all tasks. So add the filter() method in the task list before map() method: .filter(FILTER_MAP[filter]).
...
2) Declare the new useRef hook with listHeadingRef constant just above the return statement of App(). The default value is 'null'.
...
4) Now add the listHeadingRef constant as the 'ref' attribution to the same h2 element.
...
7) Under the function declare the prevTasksLength constant. usePrevious(task.length) will track here the length of the tasks.
8) Now finally declare the useState hook just above the return statement of App(). That hook has to compare the current and the previous states in terms of task.length. By the help of useRef it has to focus on the delete button.
